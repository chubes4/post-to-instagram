...
class PTI_Auth_Handler {
    ...
    public function __construct() {
        error_log('PTI_Auth_Handler instantiated');
        add_action( 'init', array( $this, 'handle_oauth_redirect' ), 5 ); // Early hook for redirect handling

        // AJAX handlers (can be deprecated later if fully replaced by REST, or kept for other uses)
        // add_action( 'wp_ajax_pti_save_app_credentials', array( $this, 'ajax_save_app_credentials' ) ); // This will be handled by REST API
        // add_action( 'wp_ajax_pti_check_auth_status', array( $this, 'ajax_check_auth_status' ) ); // This will be handled by REST API
        add_action( 'wp_ajax_pti_disconnect_account', array( $this, 'ajax_disconnect_account' ) );
    }
    ...
    public function handle_oauth_redirect() {
        ...
        if ( ! $stored_state || $stored_state !== $state || ! wp_verify_nonce( $state, self::OAUTH_STATE_NONCE_ACTION ) ) {
            ...
            error_log('Post to Instagram: OAuth state verification failed.');
        ...
        if ( ! self::is_configured() ) {
            error_log('Post to Instagram: OAuth redirect received but App ID/Secret not configured.');
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'Post to Instagram: Error exchanging code for short-lived token: ' . $response->get_error_message() );
        ...
        if ( empty( $data['access_token'] ) || empty( $data['user_id'] ) ) {
            error_log( 'Post to Instagram: Short-lived token or user_id not found in response: ' . $body );
        ...
        if ( is_wp_error( $long_lived_response ) ) {
            error_log( 'Post to Instagram: Error exchanging short-lived for long-lived token: ' . $long_lived_response->get_error_message() );
        ...
        if ( empty( $long_lived_data['access_token'] ) ) {
            error_log( 'Post to Instagram: Long-lived token not found in response: ' . $long_lived_body );
        ...
        // Store the long-lived token and user ID
    ...
    /**
     * AJAX handler to disconnect the Instagram account.
     */
    public function ajax_disconnect_account() {
        check_ajax_referer( 'pti_disconnect_nonce', 'nonce' );

        if ( ! current_user_can( 'manage_options' ) ) { // Or a more specific capability
            wp_send_json_error( array( 'message' => __( 'You do not have permission to disconnect this account.', 'post-to-instagram' ) ) );
        }

        // Clear App ID, Secret, and Auth Details
        pti_update_option( 'app_id', '' );
        pti_update_option( 'app_secret', '' );
        pti_update_option( 'auth_details', array() );

        wp_send_json_success( array( 'message' => __( 'Account disconnected successfully.', 'post-to-instagram' ) ) );
    }
} 